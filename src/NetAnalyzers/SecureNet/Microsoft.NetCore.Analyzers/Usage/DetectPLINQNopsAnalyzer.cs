...
namespace SecureNet.Category.Usage
{
    ...
    {
        internal const string RuleId = "UA2250";
        ...
        private static readonly LocalizableString s_localizableMessageDefault = new LocalizableResourceString(nameof(AnalyzerResources.DetectPLINQNopsMessage), AnalyzerResources.ResourceManager, typeof(AnalyzerResources));
        private static readonly LocalizableString s_localizableDescription = new LocalizableResourceString(nameof(AnalyzerResources.DetectPLINQNopsDescription), AnalyzerResources.ResourceManager, typeof(AnalyzerResources));
        ...
        public override void Initialize(AnalysisContext context)
        {
            ...
            {
                if (!ctx.Compilation.TryGetOrCreateTypeByMetadataName(WellKnownTypesEx.SystemLinqParallelEnumerable, out var parallelEnumerable))
                {
                    return;
                }
        ...
        private static void AnalyzeSymbol(SyntaxNodeAnalysisContext context, ImmutableHashSet<ISymbol> asParallelSymbols, ImmutableHashSet<ISymbol> toArraySymbols, ImmutableHashSet<ISymbol> toListSymbols)
        {
            ...
            if (!parentForEach.Expression.IsEquivalentTo(invocation) && //Last call is AsParallel
                (!(parentForEach.Expression is MemberAccessExpressionSyntax mem) || !s_knownCalls.Contains(mem.Name.Identifier.ValueText))) //Or ToList and ToArray. Compare by string is safe as we compare by type earlier
            {
                return;
            }
